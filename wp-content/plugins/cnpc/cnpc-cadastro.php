...
class CNPC_Cadastro
{
	...
	function shortcode_pessoa_fisica( $args )
	{
		...
		if( !$CNPC->inscricoes_abertas() )
		{
			...
			$output .= '<p style="font-size:14px">Reabertura das inscrições - de 22 a 26 de agosto de 2012.</p>
<p>Comunicamos que foram reabertas as inscrições do processo eleitoral  para renovação dos Colegiados Setoriais que compõem o Conselho Nacional de Política Cultural – CNPC.</p>
<p>O prazo de reabertura será de cinco dias, no período de 22 a 26 de agosto de 2012.</p>
<p style="text-height:bold"><a href="http://cnpc.cultura.gov.br">CLIQUE AQUI E ACESSE SEU CADASTRO</a></p>';
			$output .= '</div>';
		...
		if( !empty( $user_ID ) )
		{
			...
			else
			{
				...
				$declaracao_veracidade 			 = null;
				$declaracao_acompanhamento		 = null;
				$declaracao_pnc					 = null;
				$declaracao_das					 = null;
				$declaracao_candidato_veracidade = null;
				$candidatura    				 = null;
		...
		if( wp_verify_nonce( $nonce, 'cnpc_pessoa_fisica' ) )
		{
	...
	function update_pessoa_fisica()
	{
		...
		// verificar se o envio foi feito pelo formulário identificado
		if( !wp_verify_nonce( $check_nonce, 'cnpc_pessoa_fisica' ) )
			return false;
